...
work5=$(herbstclient tag_status | tr -s '[:blank:]' '\n' |sed -n 6p)
...
wo5=$(herbstclient tag_status | tr -s '[:blank:]' '\n'| grep \: | grep 5)
...
elif [ "$work4" == "$work_act" ] ; then
    wc4="active_workspace"
else 
    wc5="active_workspace"
fi
}
if [ "$wo1" == ":1" ] ; then
...
if [ "$wo5" == ":5" ] ; then
wc5="occupied_workspace"
fi

echo "(box :class \"workspaces\" :orientation \"h\"    :halign \"start\" :spacing \"15\"     :space-evenly \"false\"  (button  :onclick \"herbstclient use 1\"     :class \"$wc1\"     \"♧\" ) (button :onclick \"herbstclient use 2\"  :class \"$wc2\"  \"♢\") (button  :onclick \"herbstclient use 3\"   :class \"$wc3\"  \"/\" ) (button :onclick \"herbstclient use 4 \"  :class \"$wc4\"     \"♡\" ) (button :onclick \"herbstclient use 5\"   :class \"$wc5\"    \"♢\" ))"
